...
% Pure MATLAB implementation - Version 2.0
% 
% IMPROVED OBJECTIVE FUNCTION:
% J = (T/2) * sum(w_k * [||h_ref - h||^2_Q]) + (2/T) * sum(w_k * ||D*u||^2_R)
%
% Key improvements:
% 1. Delta_u penalty menggunakan Chebyshev differentiation matrix
% 2. Constraint perubahan kontrol: d_umin <= (2/T)*sum(D_kj*u_j) <= d_umax
% 3. Objective function sesuai dengan formula di dokumen
%
clc
clear all
close all
...
N = 20; % number of Chebyshev collocation points
...
% Weights (ADJUSTED - mengikuti formula dari dokumen)
w_position = 1e-1;      % Bobot Q untuk tracking error
w_control = 0;          % Tidak digunakan (diganti dengan delta_u)
w_orientation = 1e-1;   % Bobot Q untuk heading error
w_terminal = 50;        % Terminal cost weight
...
while (distance_to_destination > distance_condition && mpciter < simulation_time/time_sampling)
    ...
    % Solve Chebyshev NMPC (pass D matrix)
    [u_opt, X_opt] = solve_chebyshev_nmpc(x0, reference_pose, N, tau, D, w, ...
        ...
        w_position, w_control, w_orientation, w_terminal, D);
...
function [u_opt, X_opt] = solve_chebyshev_nmpc(x0, ref, N, tau, D, w, ...
    ...
    w_pos, w_ctrl, w_orient, w_term, D_matrix)
    ...
    % Objective and constraints (use D_matrix passed as parameter)
    obj_fun = @(z) objective_function(z, N, w, T, ref, w_pos, w_ctrl, w_orient, w_term, D_matrix);
    nonlcon = @(z) nonlinear_constraints(z, N, D_matrix, T, x0, os_A, os_B, os_surge, rrot_min, rrot_max);
...
function J = objective_function(z, N, w, T, ref, w_pos, w_ctrl, w_orient, w_term, D)
    n_states = 5;
    ...
    % Weight untuk delta u (perubahan kontrol)
    w_delta_u = 1e-2; % Sesuai dengan bobot R dalam formula
    
    % Stage cost dengan quadrature Chebyshev
    for i = 0:N
        x_ship = X(3, i+1);
        y_ship = X(4, i+1);
        psi = X(5, i+1);
        ...
        % Stage cost: ||h_ref - h||^2_Q
        stage_cost = w_pos * (pos_error' * pos_error) + ...
                     w_orient * heading_error^2;
        
        % Add with quadrature weight
        J = J + w(i+1) * stage_cost;
    end
    
    % Scale by time horizon: (T/2) * sum(...)
    J = (T/2) * J;
    
    % DELTA U TERM: (2/T) * sum(w_i * ||D_ki * u_j||^2_R)
    % Ini adalah perubahan kontrol menggunakan matriks diferensiasi Chebyshev
    for i = 0:N
        % Hitung du/dtau menggunakan Chebyshev differentiation
        du_dtau = 0;
        for j = 0:N
            du_dtau = du_dtau + D(i+1, j+1) * U(j+1);
        end
        
        % Transform ke du/dt: du/dt = (2/T) * du/dtau
        du_dt = (2/T) * du_dtau;
        
        % Add to cost with quadrature weight: (T/2) * w_i * ||du/dt||^2
        J = J + (T/2) * w(i+1) * w_delta_u * du_dt^2;
    end
...
function [c, ceq] = nonlinear_constraints(z, N, D, T, x0, os_A, os_B, os_surge, rrot_min, rrot_max)
    ...
    % Collocation equations: sum(D_kj * s_j) = (T/2) * f(s_k, u_k)
    for k = 1:N
        x_dot = zeros(n_states, 1);
        for j = 0:N
            x_dot = x_dot + D(k+1, j+1) * X(:, j+1);
        end
        % Transform: d/dtau = (T/2) * d/dt -> d/dt = (2/T) * d/dtau
        x_dot = (2/T) * x_dot;
    ...
    % === INEQUALITY: Control rate constraints ===
    c = [];
    
    % d_umin <= (2/T) * sum(D_kj * u_j) <= d_umax
    for k = 1:N
        % Compute du/dtau using Chebyshev differentiation
        du_dtau = 0;
        for j = 0:N
            du_dtau = du_dtau + D(k+1, j+1) * U(j+1);
        end
        
        % Transform to du/dt: du/dt = (2/T) * du/dtau
        du_dt = (2/T) * du_dtau;
        
        % Upper bound: du/dt <= rrot_max
        c = [c; du_dt - rrot_max];
        
        % Lower bound: du/dt >= rrot_min -> -du/dt <= -rrot_min
        c = [c; -du_dt - (-rrot_min)];
    end
end
